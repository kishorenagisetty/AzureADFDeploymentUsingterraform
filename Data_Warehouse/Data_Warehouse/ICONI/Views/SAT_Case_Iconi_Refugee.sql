CREATE VIEW [ICONI].[SAT_Case_Iconi_Refugee] 
AS 
-- ===============================================================
-- Author:	Sagar Kadiyala
-- Create date: 04/01/2024
-- Ticket Ref: #30330
-- Name: [ICONI].[SAT_Case_Iconi_Refugee] 
...
SELECT CONCAT_WS('|', 'ICONI', C.engagement_id) AS CaseKey
     ...
     , C.eng_tran_individual_id                 AS TranIndividualId
     , C.eng_tran_status_1                      AS TranStatus
     , C.eng_provider_name                      AS ProviderName
     , C.eng_info_refugee_contract_area         AS ContractArea
     , C.eng_info_refugee_nationality           AS Nationality
     , C.eng_outcome                            AS Outcome
--C.eng_info_employability_induction_pack_issued AS PackIssued, --Not Availble in the source DB so making as Null
     , C.ValidFrom
     , C.ValidTo
     , C.IsCurrent
