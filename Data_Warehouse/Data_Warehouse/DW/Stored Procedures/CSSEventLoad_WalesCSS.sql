...
-- Description: <WHP Wales CSS Logic Stored Proc>
-- Revisions:
-- 06/07/23 - <MK> - <25516> - <Logic changed for WorkflowEventEstimatedEndDate as it should be JobAddDate + 1 Working Day>
-- 25/08/23 - <MK> - <26478> - <CSS 11 Activity Date Switched from Start Date to Add Date>
-- 11/09/23 - <MK> - <27370> - < CSS 13 Criteria slightly changed to include more leave reasons & excule jobs without start dates >
-- 08/12/23 - <SK> - <30352> - < CSS 13A This is the new report to capture based on the final action plan >
-- 26/01/24 - <MK> - <30352> - < CSS 13A Criteria amended from Referral Date to Start Date as per SAM Harvey>
-- 15/02/24 - <MK> - <31676> - < CSS 12 Code amended to replace null job verified date with claim date>
-- 15/02/24 - <MK> - <30352> - < CSS 13 Changed Criteria from Referral Date to Prog Start Date> 
-- 12/03/24 - <MK> - <30865> - < Added Distinct Caluse to all CSSs to de-dupe the records >
...
insert into	DV.SAT_WorkFlowEvents_Meta_Core_New
			...
			,CSSName
			,RecordSource)
select	Distinct	cs8.CaseHashBin							as CaseHashBin
...
insert into	DV.SAT_WorkFlowEvents_Meta_Core_New
			...
			,CSSName
			,RecordSource)
select	Distinct	 cs9.CaseHashBin						as CaseHashBin
...
select	Distinct	tmp.CaseHashBin								as CaseHashBin
			...
			,null												as EmployeeHash
			,null												as AssignmentHashIfNeeded
			,act.ActivityAddDate								as WorkFlowEventDate -- 25/08/23 <MK> <26478> <Activity Date Switched from Start Date to Add Date>
	...
	(
		select
			...
			,CASE WHEN DATEPART(DW, DATEADD(DD, 1, sac.AssignmentAddDate)) = 7 
				  THEN 
						Cast(DATEADD(DD, 3, sac.AssignmentAddDate) as Date)
				  WHEN DATEPART(DW, DATEADD(DD, 1, sac.AssignmentAddDate)) = 6  -- 24/07/23 <MK> <26478> <Added for working day calculation>
				  THEN 
...
(	-- Get all workplace plan activities against a case
	select distinct
		  lcav.CaseHash
		--, cast(saac.ActivityStartDate as date)	as ActivityStartDate  -- 24/07/23 <MK> <26478> <Criteria changed to use activity start date instead of completion date>
		, cast(saac.ActivityAddDate as date)	  as ActivityAddDate  -- 25/08/23 <MK> <26478> <Criteria changed to use activity Add date instead of Start date>
	...
	where saac.ActivityName like '%Work%place%Plan%Appointment%'		-- 24/07/23 <MK> <26478> <Wildcard search criteria changed to capture missing appointments>
) act	on asg.CaseHash = act.CaseHash and act.ActivityAddDate >= asg.WorkFlowEventEstimatedStartDate and act.ActivityAddDate < asg.WorkFlowEventEstimatedEndDate -- 24/07/23 <MK> <26478> <Criteria changed slightly as discussed with Sam H>
where	tmp.ProgrammeName = 'WHP Wales';
...
select distinct	tmp.CaseHashBin								as CaseHashBin
			...
			,case when bac.BarrierStartDate > isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) -- 15/02/24 <MK> <31676>
				  then cast(bac.BarrierStartDate as date)				
				  else isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) -- 15/02/24 <MK> <31676>
			end												as BarrierStartDate 
			,dateadd(day,639
					,cast(isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) as date)) as StartDate639 -- 15/02/24 <MK> <31676>
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
join		DV.LINK_Case_Assignment							lca on lca.CaseHash			= tmp.CaseHashBin
join		DV.SAT_Assignment_Adapt_Core					sac on sac.AssignmentHash	= lca.AssignmentHash and sac.IsCurrent = 1
...
and 		tmp.StartDate is not null
and			tmp.LeaveDate is null
and			isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) is not null -- 15/02/24 <MK> <31676>
and			isnull(sac.AssignmentStartClaimYear,'') not like 'DNC%'
and 		isnull(sac.AssignmentLeaveDate, getdate()) > isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) -- 15/02/24 <MK> <31676>
and 		bac.BarrierEndDate > isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) -- Only pick barriers which continue until after the job has started
and 		((coalesce(nullif(mu1.Name,''),nullif(mu1.Description,''),'Unknown') like '%Disengage%')				
			  or (coalesce(nullif(mu1.Name,''),nullif(mu1.Description,''),'Unknown') like '%suspend engagement%'))
)
,JobVerifiedDate as (
...
select		tmp.CaseHashBin									   as CaseHashBin
			,tmp.CaseHash									   as CaseHash
			,sac.AssignmentHash								   as AssignmentHash
			,'Job Verified / Claimed Date'							   as ActivityName
			,cast(isnull(sac.AssignmentStartVerificationDate, sac.AssignmentStartClaimDate) as date) as ActivityDate -- 15/02/24 <MK> <31676>
			,lad.JobLeaveAddDate							   as JobLeaveDate
			,null											   as BarrierStartDate
			,tmp.RecordSource								   as RecordSource
from		stg.Meta_WorkflowEventsStaging					   tmp
...
where		jvd.ActivityName = 'Job Verified / Claimed Date'
...
select		jvd.CaseHashBin									as CaseHashBin
			...
			,jvd.RecordSource								as RecordSource
from		JobVerifiedDate									jvd
join		JobVerifiedDate									jvd1 on jvd1.AssignmentHash = jvd.AssignmentHash 	and jvd1.ActivityName = 'Job Verified / Claimed Date'
...
and			jvd.ActivityName = 'Job Verified / Claimed Date'
-- Do not pick up activities which occurred while the participant was disengaged
and not exists (select		dis.CaseHash
...
from (
	...
	from
		...
		left join	GetWorkflowActivity								wf2 on wf2.AssignmentHash = wf1.AssignmentHash			-- Get next activity which is within the current due date range
																	   and wf2.ActivityDueDate between 
																		   case when wf1.ActivityName in ('Job Verified / Claimed Date', 'Barrier End Date') 
																				then wf1.ActivityDate
																				else wf1.NextActivityDateWithinDueDate1d
																		   end 
																		   and wf1.NextActivityDateWithinDueDate10wd
...
As (
				...
				where CorrespondenceMethod = 1234
				AND   CorrespondenceNotes = 'Bot Successfully updated Working Pattern'
				Group by lcc.CaseHash
)
...
insert into	DV.SAT_WorkFlowEvents_Meta_Core_New
			...
			,CSSName
			,RecordSource)
select	Distinct	cs12.CaseHashBin						as CaseHashBin
			...
			-- For WorkFlowEventEstimatedStartDate, get previous WorkFlowEventDate. If null then get previous WorkFlowEventEstimatedEndDate.
			,case when cs12.ActivityName in ('Job Verified / Claimed Date', 'Barrier End Date') then cs12.ActivityDate
...
Where		Not Exists (Select 1 from HMRC_PING_BOTs ping Where ping.CaseHash = cs12.CaseHash);
...
from (
	...
	join		(select		ass.CaseHash						as CaseHash
				...
				from		(select		tmp.CaseHash
										,case	when (lad.JobLeaveAddDate <= dateadd(dd,456,tmp.StartDate) and sc1.CaseHash is null) or (lca.CaseHash is null and dateadd(dd,456,tmp.StartDate) <= getdate())	-- OutOfWorkDay456
												...
												else   dateadd(dd,456,tmp.StartDate) -- 11/09/23 <MK> <27370>
										end									as DueDate
							from		stg.Meta_WorkflowEventsStaging		tmp
							left join	(Select		ca.CaseHash, ca.AssignmentHash
										 From		DV.LINK_Case_Assignment						ca		
										 join		DV.SAT_Assignment_Adapt_Core				AAC on ca.AssignmentHash	= AAC.AssignmentHash and AAC.IsCurrent = 1
										 Where		AAC.AssignmentStartClaimDate IS NOT NULL
										 Group by	ca.CaseHash, ca.AssignmentHash
										 )									lca on lca.CaseHash			= tmp.CaseHashBin    -- 11/09/23 <MK> <27370>
							left join	AssignmentLeaveAddedDate			lad on lad.AssignmentHash	= lca.AssignmentHash
							...
							and			tmp.StartDate is not null
							and 		(tmp.LeaveDate is null 
										 or tmp.LeaveReason in ('Particpant Returned to DWP - End of Provision','Deceased (Early Completer)','Early Leaver','Early Leaver – Employment Ceased','Health Issues ie Terminal Condition','Participant Moved Out of UK')) -- 11/09/23 MK <27370>
	...
	where		tmp.ProgrammeName = 'WHP Wales'
	and			tmp.StartDate <= '2022-10-31'  -- 15/02/24 <MK> <30352>
	) qry
	)qry2
	Where		WorkFlowEventEstimatedStartDate <= convert(Date,getdate()) -- 11/09/23 <MK> <27370>
	And			InOutWork = 'Unemployed'	-- 11/09/23 <MK> <27370> 
	;
...
-- 08/12/23 - <SK> - <30352> Started
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
-- Wales WHP CSS13A Rules -------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
with AssignmentLeaveAddedDate as (
...
select		CaseHashBin
			...
			,
				...
				from   
					(
					select top(( datediff(day,dateadd(week,-4,WorkFlowEventEstimatedEndDate),WorkFlowEventEstimatedEndDate))+0 ) [Date]
					from   DW.d_date
					where  [Date] <= WorkFlowEventEstimatedEndDate
						--and is_business_day = 1
						and [Date] != '1900-01-01'
...
from (
	select		tmp.CaseHashBin									as CaseHashBin
				,tmp.CaseHash									as CaseHash
				,cs13.DocUploadedDate							as WorkFlowEventDate
				---- StartDate should be 20wd earlier if Due date is scenario1 or 
				,tmp.StartDate									as WorkFlowEventEstimatedStartDate 
				,wfe.DueDate									as WorkFlowEventEstimatedEndDate
				,tmp.InOutWork									as InOutWork
				,'WHP Wales  -  CSS13A'							as CSSName
				,tmp.RecordSource								as RecordSource
	from		stg.Meta_WorkflowEventsStaging					tmp	
	join		(select		ass.CaseHash						as CaseHash
							--We are using max because we don't have Document by Assignment so the grain has to be at 'Case' level to match the success criteria
				...
				from		(select		tmp.CaseHash
										,case	when (lad.JobLeaveAddDate <= dateadd(dd,456,tmp.StartDate) and sc1.CaseHash is null) or (lca.CaseHash is null and dateadd(dd,456,tmp.StartDate) <= getdate())	-- OutOfWorkDay456
												...
												then (select	max(gdt.[Date])	as [Date]
													  from	(select top (select	wfc.WorkFlowEventSLADuration *1
																			from	META.SAT_WorkFlowEventType_Core wfc
																			where	wfc.WorkFlowEventProgramme = 'WHP Wales'
																			and		wfc.WorkFlowEventCSSRelated = 'CSS13A'
																			union select 1)   ddd.[Date]
																from		DW.d_date ddd
																where		ddd.is_business_day = 1
																and			ddd.[Date] != '1900-01-01'
																and			ddd.[Date] > lad.JobLeaveAddDate
																order by	ddd.[Date]) gdt)
												else   dateadd(dd,456,tmp.StartDate) 
										end									as DueDate
							from		stg.Meta_WorkflowEventsStaging		tmp
							left join	(Select		ca.CaseHash, ca.AssignmentHash
										 From		DV.LINK_Case_Assignment						ca		
										 ...
										 Where		AAC.AssignmentStartClaimDate IS NOT NULL
										 Group by	ca.CaseHash, ca.AssignmentHash
										 )									lca on lca.CaseHash			= tmp.CaseHashBin    
							left join	AssignmentLeaveAddedDate			lad on lad.AssignmentHash	= lca.AssignmentHash
							left join	Scenario1InWork						sc1 on sc1.CaseHash			= tmp.CaseHash
							left join	Scenario2InWork						sc2 on sc2.CaseHash			= tmp.CaseHash
							where		tmp.ProgrammeName = 'WHP Wales'
							and			tmp.StartDate is not null
							and 		(tmp.LeaveDate is null 
										 or tmp.LeaveReason in ('Particpant Returned to DWP - End of Provision','Deceased (Early Completer)','Early Leaver','Early Leaver – Employment Ceased','Health Issues ie Terminal Condition','Participant Moved Out of UK')) 
										--	  Participant is out of work between program start date and Day 456 (Scneario 1)
										-- or participant is out of work between Day 456 and 639 (Scneario 2)
							and			(	 (lad.JobLeaveAddDate <= dateadd(dd,456,tmp.StartDate) and sc1.CaseHash is null)
										  or (lad.JobLeaveAddDate > dateadd(dd,456,tmp.StartDate)  and lad.JobLeaveAddDate < dateadd(dd,639,tmp.StartDate) and sc2.CaseHash is null)
	...
	left join	(select		tmp.CaseHash						as CaseHash
				...
				left join	DV.LINK_Case_DocumentUpload			cdu on cdu.CaseHash = tmp.CaseHashBin
				left join	DV.SAT_DocumentUpload_Adapt_Core	ddu on ddu.DocumentUploadHash = cdu.DocumentUploadHash and ddu.IsCurrent = 1
				where        (isnull(ddu.DocumentName,'')        like '%Final%Action%Plan%'
				or            isnull(ddu.DocumentDescription,'') like '%Final%Action%Plan%')
				group by	tmp.CaseHash)						cs13 on cs13.CaseHash	= tmp.CaseHash		
	where		tmp.ProgrammeName = 'WHP Wales'
	and			tmp.StartDate >= '2022-11-01' -- 26/01/24 <MK> <30352>
	) qry
	)qry2
	Where		WorkFlowEventEstimatedStartDate <= convert(Date,getdate()) 
	And			InOutWork = 'Unemployed'	 
	;
	
set @LoadStartMessage = concat('Wales CSS 13A Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
-- 08/12/23 - <SK> - <30352> Ended
Go