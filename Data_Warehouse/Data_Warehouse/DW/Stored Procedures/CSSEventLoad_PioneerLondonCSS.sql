CREATE PROC [DW].[CSSEventLoad_PioneerLondonCSS] AS

-- =======================================================================================================================================================
-- Author: Sagar Kadiyala
-- Modified date: 02/10/2023
-- Ticket Reference:  #28516
-- Description: Pioneer London CSS Logic Stored Proc
-- Revisions:
-- 02/10/2023 - SK - 28516 - Created the SP to process the data for the Pioneer London CSS
-- =======================================================================================================================================================
...
where		CSSName like '%Pioneer London%';

--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
-- Pioneer London CSS1 Rules --------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select					wfc.WorkFlowEventSLADuration *1
									 ...
									 where					wfc.WorkFlowEventProgramme = 'Pioneer London'
			...
			,'Pioneer London  -  CSS1'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
where		tmp.ProgrammeName = 'Pioneer London';

set @LoadStartMessage = concat('Pioneer London CSS 1 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select			wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS2'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
...
set @LoadStartMessage = concat('Pioneer London CSS 2 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select			wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS3'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
left join	(select		lca.CaseHash						as CaseHash
						,min(sac.ActivityCompleteDate)		as WorkFlowEventDate
...
set @LoadStartMessage = concat('Pioneer London CSS 3 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select					wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS4'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
...
set @LoadStartMessage = concat('Pioneer London CSS 4 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
and				tmp.ProgrammeName = 'Pioneer London'
...
select			cast(0x0 as binary(32))														as CaseHashBin
				...
				,null																		as InOutWork
				,'Pioneer London  -  CSS5'													as CSSName
...
set @LoadStartMessage = concat('Pioneer London CSS 5 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		cs6.CaseHashBin									as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer London  -  CSS6'						as CSSName
			,cs6.RecordSource								as RecordSource
from		CSS6_Complete									cs6;

set @LoadStartMessage = concat('Pioneer London CSS 6 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		c7b.CaseHashBin									as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer London  -  CSS7'						as CSSName
			,c7b.RecordSource								as RecordSource
from		CSS7_Complete									c7b;

set @LoadStartMessage = concat('Pioneer London CSS 7 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS8'						as CSSName
			,tmp.RecordSource								as RecordSource
from stg.Meta_WorkflowEventsStaging							tmp
left join
(
...
set @LoadStartMessage = concat('Pioneer London CSS 8 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select					wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate		 
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS9'						as CSSName
			,tmp.RecordSource								as RecordSource
from stg.Meta_WorkflowEventsStaging							tmp
left join
(
...
set @LoadStartMessage = concat('Pioneer London CSS 9 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select			wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS10'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
...
set @LoadStartMessage = concat('Pioneer London CSS 10 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select			wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS11'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
left join	(select		lca.CaseHash						as CaseHash
						,min(sac.ActivityCompleteDate)		as ActivityCompleteDate
...
set @LoadStartMessage = concat('Pioneer London CSS 11 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS12'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
...
set @LoadStartMessage = concat('Pioneer London CSS 12 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,tmp.ProjectedLeaveDate							as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS14'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
left join	(select		lca.CaseHash						as CaseHash
						,min(sac.ActivityCompleteDate)		as ActivityCompleteDate
...
set @LoadStartMessage = concat('Pioneer London CSS 14 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select					wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate		 
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS15'						as CSSName
			,tmp.RecordSource								as RecordSource
from stg.Meta_WorkflowEventsStaging							tmp
left join
(
...
set @LoadStartMessage = concat('Pioneer London CSS 15 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		cs16.CaseHashBin								as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer London  -  CSS16'						as CSSName
			,cs16.RecordSource								as RecordSource
from		CSS16_Complete									cs16;


set @LoadStartMessage = concat('Pioneer London CSS 16 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,tmp.InOutWork									as InOutWork
			,'Pioneer London  -  CSS17'						as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
left join	
(
...
set @LoadStartMessage = concat('Pioneer London CSS 17 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
Go
