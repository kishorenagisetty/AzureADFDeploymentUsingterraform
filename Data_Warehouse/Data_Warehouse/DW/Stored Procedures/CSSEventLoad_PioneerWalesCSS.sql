CREATE PROC [DW].[CSSEventLoad_PioneerWalesCSS] AS
...
-- Revisions:
-- 07/09/2023 - SK - 27867 - Created the SP to process the data for the Pioneer Wales CSS
-- 08/12/2023 - SK - 30352 - CSS 13A This is the new report to capture based on the final action plan
-- =======================================================================================================================================================
...
where		CSSName like '%Pioneer Wales%';

--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
-- Pioneer Wales CSS1 Rules --------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select			wfc.WorkFlowEventSLADuration *1
									 ...
									 where			wfc.WorkFlowEventProgramme = 'Pioneer Wales'
...
where		tmp.ProgrammeName = 'Pioneer Wales';													

set @LoadStartMessage = concat('Pioneer Wales 1 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
...
select		tmp.CaseHashBin									as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select					wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)		as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork									as InOutWork
			,'Pioneer Wales  -  CSS2'							as CSSName
			,tmp.RecordSource								as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
where		tmp.ProgrammeName = 'Pioneer Wales';		

set @LoadStartMessage = concat('Pioneer Wales CSS 2 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
set @LoadStartMessage = concat('Pioneer Wales CSS 6 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
set @LoadStartMessage = concat('Pioneer Wales CSS 7 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		cs8.CaseHashBin									as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer Wales  -  CSS8'							as CSSName
			,cs8.RecordSource								as RecordSource
from		CSS8_Complete									cs8;

set @LoadStartMessage = concat('Pioneer Wales CSS 8 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		 cs9.CaseHashBin								as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer Wales  -  CSS9'							as CSSName
			,cs9.RecordSource								as RecordSource
from		CSS9_Complete									cs9;


set @LoadStartMessage = concat('Pioneer Wales CSS 9 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		 tmp.CaseHashBin									 as CaseHashBin
			...
			,(select	max(gdt.[Date])	as [Date]
			 from		(select top (select						 wfc.WorkFlowEventSLADuration *1
						 ...
						 order by	ddd.[Date]) gdt)			 as WorkFlowEventEstimatedEndDate
			,tmp.InOutWork										 as InOutWork
			,'Pioneer Wales  -  CSS10'								 as CSSName
			,tmp.RecordSource									 as RecordSource
from		stg.Meta_WorkflowEventsStaging						 tmp
...
and		tmp.ProgrammeName = 'Pioneer Wales'
group by tmp.CaseHashBin								
		,tmp.CaseHash										
		,sac.ActivityDueDate								
		,tmp.InOutWork										
...
set @LoadStartMessage = concat('Pioneer Wales CSS 10 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		tmp.CaseHashBin										as CaseHashBin
			...
			,null												as EmployeeHash
			,null												as AssignmentHashIfNeeded
			,act.ActivityAddDate								as WorkFlowEventDate 
			...
			,tmp.InOutWork										as InOutWork
			,'Pioneer Wales  -  CSS11'								as CSSName
			,tmp.RecordSource									as RecordSource
from		stg.Meta_WorkflowEventsStaging					tmp
join
	...
	(
		select
			...
			,CASE WHEN DATEPART(DW, DATEADD(DD, 1, sac.AssignmentAddDate)) = 7 
				  ...
				  WHEN DATEPART(DW, DATEADD(DD, 1, sac.AssignmentAddDate)) = 6  
				  THEN 
...
(	-- Get all workplace plan activities against a case
	select distinct
		  lcav.CaseHash
		--, cast(saac.ActivityStartDate as date)	as ActivityStartDate  
		, cast(saac.ActivityAddDate as date)	  as ActivityAddDate  
	...
	where saac.ActivityName like '%Work%place%Plan%Appointment%'		
) act	on asg.CaseHash = act.CaseHash and act.ActivityAddDate >= asg.WorkFlowEventEstimatedStartDate and act.ActivityAddDate < asg.WorkFlowEventEstimatedEndDate 
where	tmp.ProgrammeName = 'Pioneer Wales';

set @LoadStartMessage = concat('Pioneer Wales CSS 11 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
select		cs12.CaseHashBin								as CaseHashBin
			...
			,null											as InOutWork
			,'Pioneer Wales  -  CSS12'							as CSSName
...
set @LoadStartMessage = concat('Pioneer Wales CSS 12 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();
...
from (
	select		tmp.CaseHashBin									as CaseHashBin
				...
				,tmp.InOutWork									as InOutWork
				,'Pioneer Wales  -  CSS13'							as CSSName
				,tmp.RecordSource								as RecordSource
	from		stg.Meta_WorkflowEventsStaging					tmp	
...
set @LoadStartMessage = concat('Pioneer Wales CSS 13 Time Taken = ',(select cast((getdate()-@LoadStartTime) as time(0)) '[hh:mm:ss]'))
raiserror(@LoadStartMessage, 0, 1) with nowait;
set @LoadStartTime = getdate();

-- 08/12/2023 - SK - 30352 Started
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
...
from (
	select		tmp.CaseHashBin									as CaseHashBin
				...
				,wfe.DueDate									as WorkFlowEventEstimatedEndDate
				,tmp.InOutWork									as InOutWork
				,'Pioneer Wales  -  CSS13A'							as CSSName
				,tmp.RecordSource								as RecordSource
	from		stg.Meta_WorkflowEventsStaging					tmp	
	join		(select		ass.CaseHash						as CaseHash
							--We are using max because we don't have Document by Assignment so the grain has to be at 'Case' level to match the success criteria
	...
	left join	(select		tmp.CaseHash						as CaseHash
				...
				group by	tmp.CaseHash)						cs13 on cs13.CaseHash	= tmp.CaseHash		
	where		tmp.ProgrammeName = 'Pioneer Wales'
	and			tmp.ReferralDate > '2022-11-01'
	) qry
	)qry2
